...
class Ss {
    ...
    function get_exp_date(){
        return $this->get_user_info_array()['exp_date'];
    }
    ...
    //返回具体plan
    function  get_plan(){
        $data = $this->db->select("ss_code_type","code_type_name",[
            "code_type" => $this->get_user_info_array()['plan']
        ]);
        return $data[0];
    }
    ...
    //返回用户type（是否专业版）
    function get_type(){
        return $this->get_user_info_array()['type'];
    }
    ...
    function getEnableStatus(){
        return $this->getUserArray()['enable'];
    }
